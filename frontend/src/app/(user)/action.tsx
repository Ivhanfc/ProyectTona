...
export default function UserActionScreen() {
    ...
    useEffect(() => {
        const targetId = restaurant_id || 2;
        fetchFoodItems(targetId);
    }, [restaurant_id]);

    const fetchFoodItems = async (id: string | number | string[]) => {
        setIsLoading(true);
        try {
    ...
    const handleAddToCart = (item: MenuItem) => {
        ...
        setCart((prevCart) => {
            const existingIndex = prevCart.findIndex((cartItem) => cartItem.id === numericId);
